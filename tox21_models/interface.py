...
def run(props: list, save_folder: str = None, regression: bool = False):
    ...
    #df = pd.read_csv(r'data/chureh/tox21_models/tox21_models/forest_model/df_actual_outcome_binary.csv')
    ...
    if regression:
        ...
        for i in range(5):
            ...
            if not os.path.exists(model_path):
                results, best_hp = tuner.tune_regression(datasets, save_folder)
    ...
    # Make classification models
        ...
        for i in range(5):
            model_path = os.path.join(cv_model_path, str(i), 'saved_model.pb')
            if not os.path.exists(model_path):
                results, best_hp_list, best_hp_values = tuner.tune_classification(datasets, save_folder)
                with open(os.path.join(save_folder, 'cv_results.txt'), 'w') as fin:
                    for index, row in results.iterrows():
        ...
        if not os.path.exists(f'{save_folder}/chkpt'):
            model = create_model.build_classification(dataset_final, 
                    save_folder, avg_hp_dict
            )
        else:
            model = create_model.build_classification(dataset_final, 
                    save_folder, avg_hp_dict, False
            ) 
        ...
        #with open(os.path.join(os.path.dirname(interface_folder),
          ...
          #  l_b = joblib.load(f)
        labels = ['agonist','antagonist','inactive']
        ...
        f= open(directory_txt,"w+")
        f.write("This is the checkpoint in interface right after it adds the columns ")
        val_matrix_path = os.path.join(save_folder, 'validation_matrix_with_lb_2.csv')
        test_matrix_path = os.path.join(save_folder, 'test_matrix_with_lb_2.csv')
